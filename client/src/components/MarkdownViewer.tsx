...
export function MarkdownViewer({ uploadId, pageNum }: Props) {
  ...
  if (page.state === 'pending') {
    return <div className="flex flex-1 items-center justify-center text-sm text-gray-300">Waiting for OCR...</div>
  }

  if (page.state === 'error') {
    return <div className="flex flex-1 items-center justify-center text-sm text-red-400">OCR error: {page.error}</div>
  }
